# H265SliceSegmentLayerParserTest.TestSampleSlice2

- Source: `test/h265_slice_parser_unittest.cc:77`
- Macro: `TEST_F`
...
## Inputs
- Inline byte buffer `buffer` (32 bytes)
...
- Field `first_slice_segment_in_pic_flag` equals expected value `0`.
- Field `no_output_of_prior_pics_flag` equals expected value `0`.
- Field `slice_pic_parameter_set_id` equals expected value `0`.
- Field `slice_segment_address` equals expected value `192`.
- Field `slice_type` equals expected value `3`.
- Field `slice_sao_luma_flag` equals expected value `1`.
- ... plus 4 additional field/value checks in source assertions.
...
assert:
  ...
  - verify field `first_slice_segment_in_pic_flag` equals expected value `0`.
